...
public class KinectModelControllerV2 : MonoBehaviour {
	...
	public NetworkSkeletonWrapper sw;
	...
	private const int posindex = 20;//=count of all points
	private int hipLeft = 12;
	private int hipRight = 16;
	private int shoulderCenter = 2;
	private int spine = 1;
	private int shoulderRight = 8;
	private int shoulderLeft = 4;
	...
	void Start () {
		...
		_bones = new GameObject[posindex + 5] {
		...
		_baseRotation = new Quaternion[posindex];
		_boneDir = new Vector3[posindex];
		...
		for( int ii = 0; ii < posindex; ii++)
		{
			if((_nullMask & (uint)(1 << ii)) <= 0)
			{
				...
				if(ii % 4 == 3 && ((_nullMask & (uint)(1 << (ii/4) + posindex)) <= 0))
				{
					_boneDir[ii] = _bones[(ii/4) + posindex].transform.position - _bones[ii].transform.position;
				}
				//if the bone is the hip_override (at boneindex Hip_Left, get direction from average of left and right hips
				else if(ii == hipLeft && Hip_Left != null && Hip_Right != null)
				{
		...
		_chestRight -= Vector3.Project(_chestRight, _boneDir[spine]);
		//make _hipRight orthogonal to the direction of the hip override
		Vector3.OrthoNormalize(ref _boneDir[hipLeft],ref _hipRight);
	}
	...
	void RotateJoint(int bone) {
		...
		if(bone % 4 == 0)
		{
			...
			if(bone == hipLeft)
			{
				...
				target = ((sw.bonePos[player,hipLeft] + sw.bonePos[player,hipRight]) / 2F) - sw.bonePos[player,hipCenter];
			}
			//otherwise it is one of the shoulders
			else
			{
				//target = vector from shoulder_center to bone
				target = sw.bonePos[player,bone] - sw.bonePos[player,shoulderCenter];
			}
		}
		else
		{
		...
		if(bone == spine)
		{
			...
			dir = _chestRight;
			target = sw.bonePos[player,shoulderRight] - sw.bonePos[player,shoulderLeft];
		...
		else if(bone == hipLeft)
		{
			//rotate the hips so they face forward (determined by the hips)
			dir = _hipRight;
			target = sw.bonePos[player,hipRight] - sw.bonePos[player,hipLeft];
		...
		if(bone == spine)
		{
			restoreBone(_bones[hipLeft],_boneDir[hipLeft],upDir);
			restoreBone(_bones[hipLeft],_hipRight,rightDir);
		}
